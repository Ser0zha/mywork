...
public class Course {
    ...
    public Course(int maxStudents) {
        if (maxStudents <= 0) {
            throw new IllegalArgumentException("Max students must be positive"); // ДОБАВЛЕНО
        }
    ...
    public void enroll(int studentId) {
        ...
        if (isFullyEnrolled()) { //ОШИБКА ТУТ hasWaitingList()
    ...
    public void unenroll(int studentId) {
        ...
        if (enrollList.contains(studentId)) {
            enrollList.remove((Integer) studentId);// тут косяк
            if (hasWaitingList()) {
